...
class MultiSetTest {
  ...
  def testCovariance(): Unit = {
    val catSet: MultiSet[Cat]       = MultiSet(new Cat("Alice"), new Cat("Bob"))
    val animalSet: MultiSet[Animal] = catSet
    assertEquals(1, animalSet.count(new Animal("Alice")))
    assertEquals(None, animalSet.find(new Animal("Charlie")))
  }
}
